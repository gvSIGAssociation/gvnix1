...
public class GvNIXGeoConversionServiceMetadata extends
    ...
    private static final JavaType EWKTREADER_TYPE = new JavaType(
            "org.gvnix.jpa.geo.hibernatespatial.util.EWKTReader");

    private static final JavaType EWKTWRITER_TYPE = new JavaType(
            "org.gvnix.jpa.geo.hibernatespatial.util.EWKTWriter");

    private static final JavaSymbolName PARSE_WKT_METHOD = new JavaSymbolName(
            "parseWkt");

    private static final JavaSymbolName WRITE_WKT_METHOD = new JavaSymbolName(
            "writeWkt");
    ...
    public GvNIXGeoConversionServiceMetadata(String identifier,
        ...
        // Adding util instances
        builder.addMethod(getParseWktConverterMethod());
        builder.addMethod(getWriteWktMethod());
    ...
    private MethodMetadata getParseWktConverterMethod() {
        ...
        // target type
        final MethodMetadata method = methodExists(PARSE_WKT_METHOD,
                parameterTypes);
        if (method != null) {
        ...
        throwsTypes
                .add(new JavaType("com.vividsolutions.jts.io.ParseException"));
        ...
        MethodMetadataBuilder methodBuilder = new MethodMetadataBuilder(
                getId(), Modifier.PUBLIC, PARSE_WKT_METHOD, GEOMETRY_TYPE,
    ...
    private MethodMetadata getWriteWktMethod() {
        ...
        // Check if a method with the same signature already exists in the
        // target type
        final MethodMetadata method = methodExists(WRITE_WKT_METHOD,
                parameterTypes);
        if (method != null) {
            // If it already exists, just return the method and omit its
            // generation via the ITD
        ...
        // Use the MethodMetadataBuilder for easy creation of MethodMetadata
        MethodMetadataBuilder methodBuilder = new MethodMetadataBuilder(
                getId(), Modifier.PUBLIC, WRITE_WKT_METHOD, JavaType.STRING,
                parameterTypes, parameterNames, bodyBuilder);
        methodBuilder.setAnnotations(annotations);
        methodBuilder.setThrowsTypes(throwsTypes);
    ...
    private void buildParseWktMethodBody(InvocableMemberBodyBuilder bodyBuilder) {
        bodyBuilder.appendFormalLine(String.format(helper
                .getFinalTypeName(EWKTREADER_TYPE))
                + " aReader = new "
                + (helper.getFinalTypeName(EWKTREADER_TYPE)) + "();");

        bodyBuilder.appendFormalLine("return aReader.read(wkt);");
    }

    private void buildWriteWktMethodBody(InvocableMemberBodyBuilder bodyBuilder) {
        bodyBuilder.appendFormalLine(String.format(helper
                .getFinalTypeName(EWKTWRITER_TYPE))
                + " writer = new "
                + (helper.getFinalTypeName(EWKTWRITER_TYPE)) + "();");

        bodyBuilder
                .appendFormalLine("String wkt = writer.writeFormatted(geometry);");

        bodyBuilder.appendFormalLine("return wkt;");
    }
    ...
    private void buildGetPointToStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return %s(point);",
                WRITE_WKT_METHOD));
    ...
    private void buildGetStringToPointConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return (Point) %s(str);",
                PARSE_WKT_METHOD));
    ...
    private void buildGetLineStringToStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return %s(lineString);",
                WRITE_WKT_METHOD));
    ...
    private void buildGetStringToLineStringConverterMethodBody(
        ...
        bodyBuilder.appendFormalLine(String.format(
                "return (LineString) %s(str);", PARSE_WKT_METHOD));
    ...
    private void buildGetPolygonToStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return %s(polygon);",
                WRITE_WKT_METHOD));
    ...
    private void buildGetStringToPolygonConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return (Polygon) %s(str);",
                PARSE_WKT_METHOD));
    ...
    private void buildGetGeometryToStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return %s(geometry);",
                WRITE_WKT_METHOD));
    ...
    private void buildGetStringToGeometryConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format("return %s(str);",
                PARSE_WKT_METHOD));
    ...
    private void buildGetMultiLineStringToStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format(
                "return %s(multiLineString);", WRITE_WKT_METHOD));
    ...
    private void buildGetStringToMultiLineStringConverterMethodBody(
        ...
        bodyBuilder.indent();
        bodyBuilder.appendFormalLine(String.format(
                "return (MultiLineString) %s(str);", PARSE_WKT_METHOD));
