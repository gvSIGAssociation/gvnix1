...
import org.gvnix.dynamic.configuration.roo.addon.config.SafeClientDynamicConfiguration;
...
public class SafeSecurityProvider implements SecurityProvider {
    ...
    private static final String NAMESPACE_JAXWS_VALUE = "http://cxf.apache.org/jaxws";

    private static final String NAMESPACE_JAXWS = "xmlns:jaxws";

    private static final String SCHEMA_LOCATION_JAXWS = " http://cxf.apache.org/jaxws http://cxf.apache.org/schemas/jaxws.xsd";

    private static final String NAMESPACE_C_VALUE = "http://www.springframework.org/schema/c";

    private static final String NAMESPACE_C = "xmlns:c";

    private static final String SCHEMA_LOCATION = "xsi:schemaLocation";

    private static final String CLIENT_DEF = "jaxws:client";
    ...
    private static final String WSS4JOUTINTER_SAFE = "WSS4JOutInterceptorSAFE";

    private static final String EXTWSS4JOUTINTER_SAFE = "ExtLoadWSS4JOutInterceptor";

    private static final String ENTRY = "entry";

    private static final String KEY_PROPERTY = "key";

    private static final String KEYSTORE_ALIAS = "${security.SAFE.keystore.alias}";

    private static final String CALLBACK_REF = "passwordCallbackSAFE";
    ...
    private static final String MAP_ROLES_VALUE = "${security.SAFE.mapRoles}";
    ...
    public void install(JavaPackage targetPackage) {
        ...
        if (!fileExists(EXTWSS4JOUTINTER_SAFE, targetPackage)) {
            generateExtLoadWSS4JOutInterceptor(targetPackage);
        }
        ...
        modifyApplicationContext(targetPackage);
     ...
     * 
     */
    public void modifyApplicationContext(JavaPackage targetPackage) {
        ...
        config.setAttribute(NAMESPACE_JAXWS, NAMESPACE_JAXWS_VALUE);
        config.setAttribute(NAMESPACE_C, NAMESPACE_C_VALUE);
        StringBuffer schemaLocationValue = new StringBuffer(
                config.getAttribute(SCHEMA_LOCATION));
        config.removeAttribute(SCHEMA_LOCATION);
        schemaLocationValue.append(SCHEMA_LOCATION_JAXWS);
        config.setAttribute(SCHEMA_LOCATION, schemaLocationValue.toString());
        ...
        // Add clients SAFE
        // Autentication
        Element clientAutentication = document.createElement(CLIENT_DEF);
        clientAutentication.setAttribute(ID, "safeAutenticacionClient");
        clientAutentication
                .setAttribute(SERVICE_CLASS,
                        "es.gva.dgm.ayf.war.definitions.v2u00.AutenticacionArangiPortType");
        clientAutentication.setAttribute(ADDRESS, "${wsdl.SAFE.location}");

        // Authorization
        Element clientAuthorization = document.createElement(CLIENT_DEF);
        clientAuthorization.setAttribute(ID, "safeAutorizacionClient");
        clientAuthorization.setAttribute(SERVICE_CLASS,
                "es.gva.dgm.ayf.war.definitions.v2u00.AutorizacionPortType");
        clientAuthorization.setAttribute(ADDRESS,
                "${wsdl.SAFEAutorizacion.location}");

        // Children of clients
        Element interceptorAutentication = document
                .createElement("jaxws:outInterceptors");
        Element interceptorRef = document.createElement("ref");
        interceptorRef.setAttribute(BEAN, WSS4JOUTINTER_SAFE);
        interceptorAutentication.appendChild(interceptorRef);

        // clone interceptor
        Element interceptorAuthorization = (Element) interceptorAutentication
                .cloneNode(true);

        // Add interceptor to clients and clients to beans
        clientAutentication.appendChild(interceptorAutentication);
        config.appendChild(clientAutentication);

        clientAuthorization.appendChild(interceptorAuthorization);
        config.appendChild(clientAuthorization);

        // Add ExtWSS4JOutInterceptor
        Element beanWss4J = document.createElement(BEAN);
        beanWss4J.setAttribute(ID, WSS4JOUTINTER_SAFE);
        beanWss4J.setAttribute(CLASS,
                targetPackage.getFullyQualifiedPackageName().concat(".")
                        .concat(EXTWSS4JOUTINTER_SAFE));

        Element constructorWss4J = document.createElement("constructor-arg");
        Element mapWss4J = document.createElement("map");

        // Entries
        Element entryAct = document.createElement(ENTRY);
        entryAct.setAttribute(KEY_PROPERTY, "action");
        entryAct.setAttribute(VALUE_PROPERTY, "Signature");

        Element entryUsr = document.createElement(ENTRY);
        entryUsr.setAttribute(KEY_PROPERTY, "user");
        entryUsr.setAttribute(VALUE_PROPERTY, KEYSTORE_ALIAS);

        Element entrySigUsr = document.createElement(ENTRY);
        entrySigUsr.setAttribute(KEY_PROPERTY, "signatureUser");
        entrySigUsr.setAttribute(VALUE_PROPERTY, KEYSTORE_ALIAS);

        Element entrySigKey = document.createElement(ENTRY);
        entrySigKey.setAttribute(KEY_PROPERTY, "signatureKeyIdentifier");
        entrySigKey.setAttribute(VALUE_PROPERTY, "DirectReference");

        Element entrySigFile = document.createElement(ENTRY);
        entrySigFile.setAttribute(KEY_PROPERTY, "signaturePropFile");
        entrySigFile.setAttribute(VALUE_PROPERTY,
                "${security.SAFE.file.sign.location}");

        Element entryCallback = document.createElement(ENTRY);
        entryCallback.setAttribute(KEY_PROPERTY, "passwordCallbackRef");
        entryCallback.setAttribute("value-ref", CALLBACK_REF);

        mapWss4J.appendChild(entryAct);
        mapWss4J.appendChild(entryUsr);
        mapWss4J.appendChild(entrySigUsr);
        mapWss4J.appendChild(entrySigKey);
        mapWss4J.appendChild(entrySigFile);
        mapWss4J.appendChild(entryCallback);

        constructorWss4J.appendChild(mapWss4J);
        beanWss4J.appendChild(constructorWss4J);

        // Add property traceabilityId
        Element propertyTraceId = document.createElement(PROPERTY);
        propertyTraceId.setAttribute(NAME_PROPERTY, "traceabilityId");
        propertyTraceId.setAttribute(VALUE_PROPERTY,
                "${security.SAFE.traceabilityId}");

        beanWss4J.appendChild(propertyTraceId);

        config.appendChild(beanWss4J);

        // Add Callback
        Element beanCallback = document.createElement(BEAN);
        beanCallback.setAttribute(ID, CALLBACK_REF);
        beanCallback.setAttribute(CLASS, targetPackage
                .getFullyQualifiedPackageName().concat(".PasswordHandler"));

        Element propertyCallback = document.createElement("property");
        propertyCallback.setAttribute(NAME_PROPERTY, "password");
        propertyCallback.setAttribute(VALUE_PROPERTY,
                "${security.SAFE.alias.password}");

        beanCallback.appendChild(propertyCallback);

        config.appendChild(beanCallback);
    ...
    public void modifyApplicationContextSecurity(JavaPackage targetPackage) {
        ...
        Element mapRolesProperty = document.createElement(BEANS_PROPERTY);
        mapRolesProperty.setAttribute(NAME_PROPERTY, MAP_ROLES);
        mapRolesProperty.setAttribute(VALUE_PROPERTY, MAP_ROLES_VALUE);
        ...
        newSafeBeans.appendChild(mapRolesProperty);
    ...
    public void showNextSteps() {
        ...
        log.log(Level.INFO,
                "    - security.SAFE.applet.location  (** SAFE applet URL to login using certificate (Ej: 'https://pretramitaext.gva.es/SAFE/applet'))");
        log.log(Level.INFO,
                "    - security.SAFE.file.sign.location  (** Path where SAFE signature properties file is located (Ej: 'file:///user/path/SAFE/signature.properties'))");
        log.log(Level.INFO,
                "    - security.SAFE.traceabilityId  (** Id used to create the traceability head for SOAP message) ");
        log.log(Level.INFO,
